...
import bookAuthorshipService, { BookAuthorshipItem } from "../../services/bookAuthorshipService";
...
export default function BookAuthorshipListPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<BookAuthorshipItem | null>(null);

  const isPdf = (item: BookAuthorshipItem) => {
  ...
  const load = async () => {
    ...
    try {
      const res = await bookAuthorshipService.getByUser(user.id);
      const data = Array.isArray(res?.data) ? res.data : [];
      setItems(data);
    } catch (err: any) {
      setError(err?.response?.data?.message || "Kitab müəllifliyi məlumatları yüklənmədi");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <PageMeta title="Kitab müəllifliyi | Performix" description="Müəllim üçün kitab müəllifliyi siyahısı" />
      <PageBreadcrumb pageTitle="Kitab müəllifliyi" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Kitab müəllifliyim</h3>
          <button
            onClick={() => navigate("/userbookauthorship/new")}
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Hələ kitab müəllifliyi məlumatı əlavə edilməyib.</p>
        ) : (
                    ...
                    <td className="py-3 pr-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/userbookauthorship/${item.id}/edit`)}
                          ...
                          aria-label="Məlumatı redaktə et"
      ...
      <Modal isOpen={Boolean(selectedItem)} onClose={() => setSelectedItem(null)} className="m-4 w-full max-w-3xl">
        {selectedItem && (
          <div className="rounded-3xl bg-white p-5 dark:bg-gray-900 sm:p-6">
            <h3 className="mb-4 pr-12 text-lg font-semibold text-gray-800 dark:text-white">Kitab müəllifliyi detalları</h3>
