...
export default function CreateBookAuthorshipPage() {
  ...
  const loadCategories = async () => {
    ...
    try {
      const res = await evaluationParameterService.getAll("book_authorship");
  ...
  useEffect(() => {
    const loadRecordForEdit = async () => {
      ...
      setError("");
      try {
        const res = await bookAuthorshipService.getById(recordId, user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && Number.isFinite(recordId)) {
        await bookAuthorshipService.update({
          id: recordId,
          ...payload
        });
      } else {
        await bookAuthorshipService.create(payload);
      }

      navigate("/userbookauthorship");
    } catch (err: any) {
  ...
  return (
    <>
      <PageMeta
        title={isEditMode ? "Kitab müəllifliyini redaktə et | Performix" : "Kitab müəllifliyi əlavə et | Performix"}
        description={isEditMode ? "Mövcud kitab müəllifliyi məlumatının redaktəsi" : "Yeni kitab müəllifliyi əlavə etmə forması"}
      />
      <PageBreadcrumb pageTitle={isEditMode ? "Kitab müəllifliyini redaktə et" : "Kitab müəllifliyi əlavə et"} />
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate("/userbookauthorship")}
