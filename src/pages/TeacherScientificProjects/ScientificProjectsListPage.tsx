...
export default function ScientificProjectsListPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await scientificProjectService.getByUser(user.id);
      const data = Array.isArray(res?.data) ? res.data : [];
      setItems(data);
    } catch (err: any) {
      setError(err?.response?.data?.message || "Elmi tədqiqat layihələri yüklənmədi");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <PageMeta title="Elmi tədqiqat layihələri | Performix" description="Müəllim üçün elmi tədqiqat layihələri siyahısı" />
      <PageBreadcrumb pageTitle="Elmi tədqiqat layihələri" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Elmi tədqiqat layihələrim</h3>
          <button
            onClick={() => navigate("/userscientificprojects/new")}
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Hələ elmi tədqiqat layihəsi əlavə edilməyib.</p>
        ) : (
                    ...
                    <td className="py-3 pr-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/userscientificprojects/${item.id}/edit`)}
                          ...
                          aria-label="Layihəni redaktə et"
          ...
          <div className="rounded-3xl bg-white p-5 dark:bg-gray-900 sm:p-6">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusBadgeClass(selectedItem.status)}`}>
            ...
            <div className="mt-6 flex justify-end">
              <button onClick={() => setSelectedItem(null)} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                Bağla
              </button>
            </div>
          </div>
