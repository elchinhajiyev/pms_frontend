...
export default function BookAuthorshipMonitoringPage() {
  ...
  const handleApprove = async (item: BookAuthorshipItem) => {
    if (!user?.id) return;
  ...
  const handleReject = async (item: BookAuthorshipItem, reason: string) => {
    if (!user?.id) return;
    if (!reason.trim()) {
  ...
  const openDetailsModal = (item: BookAuthorshipItem) => {
  ...
  return (
    <>
      <PageMeta title="Elmi fəaliyyətlər monitorinq | Performix" description="Kitab müəllifliyi monitorinqi və təsdiqi" />
      <PageBreadcrumb pageTitle="Elmi fəaliyyətlər monitorinq" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Kitab müəllifliyi monitorinq</h3>
