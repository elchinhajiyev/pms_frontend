...
import scientificPublicationService, { ScientificPublicationItem } from "../../services/scientificPublicationService";
...
export default function ScientificPublicationsMonitoringPage() {
  const { user } = useAuth();

  const [items, setItems] = useState<ScientificPublicationItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [processingId, setProcessingId] = useState<number | null>(null);
  const [filterStatus, setFilterStatus] = useState("");
  const [selectedItem, setSelectedItem] = useState<ScientificPublicationItem | null>(null);
  ...
  const handleApprove = async (item: ScientificPublicationItem) => {
    if (!user?.id) return;

    setProcessingId(item.id);
    setError("");
    try {
      await scientificPublicationService.approve(item.id, user.id);
      await load();
  ...
  const handleReject = async (item: ScientificPublicationItem, reason: string) => {
    if (!user?.id) return;
    if (!reason.trim()) {
    ...
    setModalError("");
    try {
      await scientificPublicationService.reject(item.id, user.id, reason.trim());
      await load();
      if (selectedItem?.id === item.id) {
        setSelectedItem(null);
        setShowRejectBox(false);
  ...
  const openDetailsModal = (item: ScientificPublicationItem) => {
  ...
  const isPdf = (item: ScientificPublicationItem) => {
  ...
  return (
    <>
      <PageMeta title="Elmi fəaliyyətlər monitorinq | Performix" description="Elmi nəşrlər monitorinqi və təsdiqi" />
      <PageBreadcrumb pageTitle="Elmi fəaliyyətlər monitorinq" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Elmi nəşrlər monitorinq</h3>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">Gözləmədə olan sənədlər: {pendingCount}</p>
          </div>
        ...
        {error && (
          <p className="mb-4 rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 dark:bg-red-900/30 dark:text-red-300">{error}</p>
        )}
      ...
      <Modal isOpen={Boolean(selectedItem)} onClose={closeDetailsModal} className="m-4 w-full max-w-3xl">
        {selectedItem && (
          <div className="rounded-3xl bg-white p-5 dark:bg-gray-900 sm:p-6">
            <h3 className="mb-4 pr-12 text-lg font-semibold text-gray-800 dark:text-white">Elmi nəşr detalları</h3>

            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Başlıq</p>
            ...
            <div className="mt-4 space-y-3">
              {selectedItem.summary && (
                <div>
                  ...
                  <p className="rounded-lg border border-gray-200 bg-gray-50 p-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">{selectedItem.summary}</p>
                </div>
              )}
              ...
              {selectedItem.link && (
                <div>
                  ...
                  <a href={selectedItem.link} target="_blank" rel="noreferrer" className="text-sm text-blue-600 hover:text-blue-700 dark:text-blue-400">
                    {selectedItem.link}
                  </a>
                </div>
              )}
            ...
            {modalError && (
              <p className="mt-4 rounded-lg bg-red-100 px-3 py-2 text-sm text-red-700 dark:bg-red-900/30 dark:text-red-300">{modalError}</p>
            )}
            ...
            <div className="mt-6 flex flex-wrap items-center justify-end gap-2">
              <button onClick={closeDetailsModal} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                Bağla
              </button>
