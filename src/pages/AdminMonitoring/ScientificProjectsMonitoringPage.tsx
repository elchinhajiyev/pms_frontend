...
import scientificProjectService, { ScientificProjectItem } from "../../services/scientificProjectService";
...
export default function ScientificProjectsMonitoringPage() {
  ...
  const [selectedItem, setSelectedItem] = useState<ScientificProjectItem | null>(null);
  ...
  const handleApprove = async (item: ScientificProjectItem) => {
    if (!user?.id) return;
  ...
  const handleReject = async (item: ScientificProjectItem, reason: string) => {
    if (!user?.id) return;
    if (!reason.trim()) {
  ...
  const openDetailsModal = (item: ScientificProjectItem) => {
  ...
  const isPdf = (item: ScientificProjectItem) => {
  ...
  return (
    <>
      <PageMeta title="Elmi fəaliyyətlər monitorinq | Performix" description="Elmi tədqiqat layihələri monitorinqi və təsdiqi" />
      <PageBreadcrumb pageTitle="Elmi fəaliyyətlər monitorinq" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Elmi tədqiqat layihələri monitorinq</h3>
      ...
      <Modal isOpen={Boolean(selectedItem)} onClose={closeDetailsModal} className="m-4 w-full max-w-3xl">
        {selectedItem && (
          <div className="rounded-3xl bg-white p-5 dark:bg-gray-900 sm:p-6">
            <h3 className="mb-4 pr-12 text-lg font-semibold text-gray-800 dark:text-white">Elmi tədqiqat layihəsi detalları</h3>
