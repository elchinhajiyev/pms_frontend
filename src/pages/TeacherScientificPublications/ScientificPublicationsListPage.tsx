...
export default function ScientificPublicationsListPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await scientificPublicationService.getByUser(user.id);
      const data = Array.isArray(res?.data) ? res.data : [];
      setItems(data);
    } catch (err: any) {
      setError(err?.response?.data?.message || "Elmi nəşrlər yüklənmədi");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <PageMeta title="Elmi nəşrlər | Performix" description="Müəllim üçün elmi nəşrlər siyahısı" />
      <PageBreadcrumb pageTitle="Elmi nəşrlər" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Elmi nəşrlərim</h3>
          <button
            onClick={() => navigate("/userscientificpublications/new")}
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Hələ elmi nəşr əlavə edilməyib.</p>
        ) : (
                    ...
                    <td className="py-3 pr-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/userscientificpublications/${item.id}/edit`)}
