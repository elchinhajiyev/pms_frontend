...
export default function CreateScientificPublicationPage() {
  ...
  const loadCategories = async () => {
    ...
    try {
      const res = await evaluationParameterService.getAll("scientific_publications");
  ...
  useEffect(() => {
    const loadRecordForEdit = async () => {
      ...
      setError("");
      try {
        const res = await scientificPublicationService.getById(recordId, user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && Number.isFinite(recordId)) {
        await scientificPublicationService.update({
          id: recordId,
          ...payload
        });
      } else {
        await scientificPublicationService.create(payload);
      }

      navigate("/userscientificpublications");
    } catch (err: any) {
  ...
  return (
    <>
      <PageMeta
        title={isEditMode ? "Elmi nəşri redaktə et | Performix" : "Elmi nəşr əlavə et | Performix"}
        description={isEditMode ? "Mövcud elmi nəşrin redaktəsi" : "Yeni elmi nəşr əlavə etmə forması"}
      />
      <PageBreadcrumb pageTitle={isEditMode ? "Elmi nəşri redaktə et" : "Elmi nəşr əlavə et"} />
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Nəşr adını daxil edin"
              />
            </div>
          ...
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate("/userscientificpublications")}
