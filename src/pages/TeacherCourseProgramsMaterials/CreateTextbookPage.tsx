...
export default function CreateTextbookPage() {
  ...
  const loadCategories = async () => {
    ...
    try {
      const res = await evaluationParameterService.getAll("textbooks");
  ...
  useEffect(() => {
    const loadRecordForEdit = async () => {
      ...
      setError("");
      try {
        const res = await teachingProgramService.getById(recordId, user.id, "textbooks");
  ...
  return (
    <>
      <PageMeta
        title={isEditMode ? "Dərsliyi redaktə et | Performix" : "Dərslik əlavə et | Performix"}
        description={isEditMode ? "Mövcud dərsliyin redaktəsi" : "Yeni dərslik əlavə etmə forması"}
      />
      <PageBreadcrumb pageTitle={isEditMode ? "Dərsliyi redaktə et" : "Dərslik əlavə et"} />
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Dərsliyin adını daxil edin"
              />
            </div>
          ...
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate("/usercoursebooks")}
