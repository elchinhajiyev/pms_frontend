...
export default function CreateMethodicalMaterialPage() {
  ...
  const loadCategories = async () => {
    ...
    try { const res = await evaluationParameterService.getAll("methodical_materials"); const data = Array.isArray(res?.data) ? res.data : []; setCategories(data); setCategoriesLoaded(true); if (!categoryId && data.length > 0) setCategoryId(String(data[0].id)); } catch { setError("Kateqoriyalar yüklənmədi"); setCategories([]); } finally { setLoadingCategories(false); }
  };
  ...
  useEffect(() => {
    const loadRecordForEdit = async () => {
      ...
      setLoadingRecord(true); setError("");
      try {
        const res = await teachingProgramService.getById(recordId, user.id, "methodical_materials");
      ...
      } catch (err: any) { setError(err?.response?.data?.message || "Məlumatlar yüklənmədi"); } finally { setLoadingRecord(false); }
    };
    loadRecordForEdit();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && Number.isFinite(recordId)) await teachingProgramService.update({ id: recordId, ...payload }); else await teachingProgramService.create(payload);
      navigate("/usercoursemethodicalmaterials");
    } catch (err: any) { setError(err?.response?.data?.message || "Yadda saxlamaq mümkün olmadı"); } finally { setSaving(false); }
  };

  return (
    <>
      <PageMeta title={isEditMode ? "Metodiki vəsaiti redaktə et | Performix" : "Metodiki vəsait əlavə et | Performix"} description={isEditMode ? "Mövcud metodiki vəsaitin redaktəsi" : "Yeni metodiki vəsait əlavə etmə forması"} />
      <PageBreadcrumb pageTitle={isEditMode ? "Metodiki vəsaiti redaktə et" : "Metodiki vəsait əlavə et"} />
      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2"><label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Başlıq *</label><input type="text" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white" placeholder="Metodiki vəsaitin adını daxil edin" /></div>
            ...
            <div><label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-300">Tədris ili *</label><select value={academicYear} onChange={(e) => setAcademicYear(e.target.value)} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="">Tədris ili seçin</option>{academicYears.map((year) => <option key={year} value={year}>{year}</option>)}</select></div>
          ...
          <div className="flex justify-end gap-3"><button type="button" onClick={() => navigate("/usercoursemethodicalmaterials")} className="rounded-lg border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">Ləğv et</button><button type="submit" disabled={saving || loadingRecord} className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-60">{saving ? "Yadda saxlanılır..." : isEditMode ? "Dəyişiklikləri yadda saxla" : "Yadda saxla"}</button></div>
        </form>
      </div>
    </>
  );
