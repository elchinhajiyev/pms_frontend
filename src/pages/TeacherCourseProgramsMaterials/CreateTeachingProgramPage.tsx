...
export default function CreateTeachingProgramPage() {
  ...
  const loadCategories = async () => {
    ...
    try {
      const res = await evaluationParameterService.getAll("course_programs");
  ...
  useEffect(() => {
    const loadRecordForEdit = async () => {
      ...
      setError("");
      try {
        const res = await teachingProgramService.getById(recordId, user.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode && Number.isFinite(recordId)) {
        await teachingProgramService.update({
          id: recordId,
          ...payload
        });
      } else {
        await teachingProgramService.create(payload);
      }

      navigate("/usercourseprograms");
    } catch (err: any) {
  ...
  return (
    <>
      <PageMeta
        title={isEditMode ? "Tədris proqramını redaktə et | Performix" : "Tədris proqramı əlavə et | Performix"}
        description={isEditMode ? "Mövcud tədris proqramının redaktəsi" : "Yeni tədris proqramı əlavə etmə forması"}
      />
      <PageBreadcrumb pageTitle={isEditMode ? "Tədris proqramını redaktə et" : "Tədris proqramı əlavə et"} />
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="md:col-span-2">
              ...
              <input
                ...
                placeholder="Proqram adını daxil edin"
              />
            </div>
            ...
            <div className="md:col-span-2">
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">Faylı yükləyin (drag and drop)</label>
              <div
                {...getRootProps()}
          ...
          <div className="flex justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate("/usercourseprograms")}
