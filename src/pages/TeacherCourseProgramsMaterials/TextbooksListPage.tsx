...
export default function TextbooksListPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await teachingProgramService.getByUser(user.id, "textbooks");
      const data = Array.isArray(res?.data) ? res.data : [];
      setItems(data);
    } catch (err: any) {
      setError(err?.response?.data?.message || "Dərsliklər yüklənmədi");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <PageMeta title="Dərsliklər | Performix" description="Müəllim üçün dərsliklər siyahısı" />
      <PageBreadcrumb pageTitle="Dərsliklər" />

      <div className="rounded-xl bg-white p-6 shadow-sm dark:bg-gray-800">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Dərsliklərim</h3>
          <button
            onClick={() => navigate("/usercoursebooks/new")}
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Hələ dərslik əlavə edilməyib.</p>
        ) : (
                    ...
                    <td className="py-3 pr-4 text-right">
                      <div className="inline-flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => navigate(`/usercoursebooks/${item.id}/edit`)}
                          ...
                          aria-label="Dərsliyi redaktə et"
      ...
      <Modal isOpen={Boolean(selectedItem)} onClose={() => setSelectedItem(null)} className="m-4 w-full max-w-3xl">
        {selectedItem && (
          <div className="rounded-3xl bg-white p-5 dark:bg-gray-900 sm:p-6">
            <h3 className="mb-4 pr-12 text-lg font-semibold text-gray-800 dark:text-white">Dərslik detalları</h3>
            ...
            <div className="mt-4 space-y-3">
              {selectedItem.summary && (
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400">Qısa məlumat</p>
