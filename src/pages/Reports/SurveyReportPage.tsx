...
import reportService, {
  SurveyQuestionReportQuestion,
  SurveyQuestionReportRow,
...
export default function SurveyReportPage() {
  ...
  const [questions, setQuestions] = useState<SurveyQuestionReportQuestion[]>([]);
  const [rows, setRows] = useState<SurveyQuestionReportRow[]>([]);
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        ...
        const [yearsRes, semestersRes, departmentsRes, facultiesRes, surveysRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        ...
        setError("");
        const response = await reportService.getSurveyQuestionReport({
        ...
        setQuestions([]);
        setRows([]);
        setError(err?.response?.data?.message || "Sorğu hesabatı yüklənmədi");
      } finally {
  ...
  }, [academicYear, semester, departmentFilter, surveyId, search]);
  ...
  const filteredSurveys = useMemo(
    () =>
      surveys.filter((survey) => {
        const yearMatch = !academicYear || String(survey.year || "") === academicYear;
        const semesterMatch =
          !semester || String(survey.semester || "").toUpperCase() === semester;
        return yearMatch && semesterMatch;
      }),
    [academicYear, semester, surveys]
  );
    ...
    return [...rows].sort((firstRow, secondRow) => {
      const firstValue = Number(
        sortConfig.key === "__total_average__"
          ...
          : firstRow.question_scores?.[sortConfig.key]
      );
      const secondValue = Number(
        sortConfig.key === "__total_average__"
          ? secondRow.total_average_score
          : secondRow.question_scores?.[sortConfig.key]
      );
  ...
  const chartTitle = useMemo(() => {
    if (selectedNames.length === 1) {
      return `${selectedNames[0]} üçün sorğu sualları üzrə orta ballar`;
    }
    if (selectedNames.length > 1) {
      return `${selectedNames.join(", ")} üçün sorğu sualları üzrə orta ballar`;
    }
    return "Sorğu sualları üzrə orta ballar";
  }, [selectedNames]);
      ...
      questions
        .map((question) => {
          const values = chartRows
            .map((row) => Number(row.question_scores?.[question.key]))
  ...
  const chartOptions = useMemo<ApexOptions>(
    () => ({
      ...
      tooltip: {
        x: {
          formatter: (_value, options) => chartData[options.dataPointIndex]?.text || "",
        },
        y: {
          formatter: (value: number) => value.toFixed(2),
        },
  ...
  const chartSeries = useMemo(
    () => [{ name: "Orta bal", data: chartData.map((item) => item.average) }],
    [chartData]
  );
  ...
  const exportRows = useMemo(
    () =>
      sortedRows.map((row, index) => {
        const questionColumns = questions.reduce((acc, question) => {
          acc[question.text] = formatScore(row.question_scores?.[question.key]);
          return acc;
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sorğu hesabatı");
    const suffix = [academicYear, semester, surveyId].filter(Boolean).join("-");
    XLSX.writeFile(workbook, `sorgu-hesabati${suffix ? `-${suffix}` : ""}.xlsx`);
  };

  const handleDownloadChart = async () => {
    const dataUri = await ApexCharts.exec("survey-report-chart", "dataURI");
    if (!dataUri?.imgURI) return;

    const link = document.createElement("a");
    link.href = dataUri.imgURI;
    link.download = "sorgu-diagrami.png";
    link.click();
  };

  const tableQuestions = questions;
  const tableMinWidth = Math.max(796 + tableQuestions.length * 92, 1020);

  return (
    <>
      <PageMeta title="Sorğu hesabatı | Performix" description="Sorğu hesabatı" />
      <PageBreadcrumb pageTitle="Sorğu hesabatı" />

      <div className="space-y-5">
        <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[170px_140px_210px_220px_minmax(170px,230px)_auto] xl:items-end">
            <div>
            ...
            <div>
              <label className="mb-1 block text-xs text-gray-500 dark:text-gray-400">Sorğu vərəqi</label>
              <select
                value={surveyId}
                onChange={(event) => setSurveyId(event.target.value)}
                className="h-10 w-full rounded-md border border-gray-300 px-3 text-sm outline-none focus:border-brand-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
              >
                <option value="">Bütün sorğular</option>
        ...
        {!academicYear ? (
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-theme-xs dark:border-gray-700 dark:bg-gray-900">
            <div className="overflow-x-auto">
              <table className="table-fixed text-xs" style={{ minWidth: tableMinWidth, width: "100%" }}>
                <colgroup>
                  ...
                  {tableQuestions.map((question) => (
                    <col key={question.key} className="w-[92px]" />
                  ))}
                ...
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-25 text-left text-xs font-normal text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300">
                    ...
                    {tableQuestions.map((question) => (
                      <th key={question.key} title={question.text} className="border-r border-gray-200 px-2 py-2 text-center font-normal dark:border-gray-700">
                        <button
                          type="button"
                          onClick={() => toggleSort(question.key)}
                          ...
                          title={question.text}
                        >
                          <span className="block truncate">{shortText(question.text)}</span>
                          <span className="flex shrink-0 flex-col text-[8px] leading-[8px] text-gray-400">
                            <span className={sortConfig?.key === question.key && sortConfig.direction === "asc" ? "text-brand-600" : ""}>▲</span>
                            <span className={sortConfig?.key === question.key && sortConfig.direction === "desc" ? "text-brand-600" : ""}>▼</span>
                          </span>
                        </button>
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                  {loadingReport ? (
                    <tr>
                      <td colSpan={tableQuestions.length + 5} className="px-4 py-10 text-center text-sm text-gray-500 dark:text-gray-400">
                        Yüklənir...
                      </td>
                    </tr>
                  ) : rows.length === 0 ? (
                    ...
                    sortedRows.map((row, index) => (
                      <tr key={row.user_id} className="bg-white transition-colors hover:bg-gray-25 dark:bg-gray-900 dark:hover:bg-gray-800/70">
                        ...
                        {tableQuestions.map((question) => (
                          <td key={`${row.user_id}-${question.key}`} className="whitespace-nowrap border-r border-gray-100 px-2 py-1.5 text-center text-gray-600 dark:border-gray-800 dark:text-gray-400">
                            {formatScore(row.question_scores?.[question.key]) || "—"}
                          </td>
                        ))}
      ...
      <Modal isOpen={chartOpen} onClose={() => setChartOpen(false)} className="m-4 w-full max-w-5xl">
        <div className="p-5 pr-14 sm:p-6 sm:pr-16">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h3 className="max-w-[720px] text-base font-medium text-gray-900 dark:text-white">{chartTitle}</h3>
            <button
              type="button"
          ...
          {chartData.length === 0 ? (
            <p className="py-10 text-center text-sm text-gray-500 dark:text-gray-400">Diagram üçün məlumat tapılmadı.</p>
          ) : (
            <div className="overflow-x-auto">
